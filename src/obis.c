...
{{{  1,   DC,   1,   8,	DC, DC}}, "counter-in",		"Zaehlerstand Wirkleistung Bezug"},
{{{  1,   DC,   2,   8,  DC, DC}}, "counter-out",	"Zaehlerstand Wirkleistung Lieferung"},

/* Easymeter */

/* ESYQ3B (Easymeter Q3B) */
{{{  1,   DC,   1,   8,   1,  DC}}, "counter-in-t1",	"Zaehlerstand Wirkleistung Bezug - Tarif 1"},
{{{  1,   DC,   1,   8,   2,  DC}}, "counter-in-t2",	"Zaehlerstand Wirkleistung Bezug - Tarif 2"},
{{{  1,   DC,   1,   8,   3,  DC}}, "counter-in-t3",	"Zaehlerstand Wirkleistung Bezug - Tarif 3"},
{{{  1,   DC,   1,   8,   4,  DC}}, "counter-in-t4",	"Zaehlerstand Wirkleistung Bezug - Tarif 4"},
{{{  1,   DC,   1,   8,   5,  DC}}, "counter-in-t5",	"Zaehlerstand Wirkleistung Bezug - Tarif 5"},
{{{  1,   DC,   1,   8,   6,  DC}}, "counter-in-t6",	"Zaehlerstand Wirkleistung Bezug - Tarif 6"},

/* ESYQ3B (Easymeter Q3C) */
{{{  1,   DC,   2,   8,   7,  DC}}, "counter-out-t7",	"Zaehlerstand Wirkleistung Bezug - Tarif 1"},
{{{  1,   DC,   2,   8,   8,  DC}}, "counter-out-t8",	"Zaehlerstand Wirkleistung Bezug - Tarif 2"},
...
obis_alias_t * obis_get_alias(obis_id_t *id) {

	obis_alias_t * alias = NULL;
	for (const obis_alias_t *it = aliases;
			it - aliases < sizeof(aliases) / sizeof(obis_alias_t); it++) {

		if (obis_compare(it->id, *id) == 0) {
			alias = (obis_alias_t *)it;
		}
	}
	return alias;
